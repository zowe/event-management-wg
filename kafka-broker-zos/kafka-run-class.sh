...
if echo "$(uname -a)" | grep -q "CYGWIN"; then
  CYGWIN=1
else
  CYGWIN=0
fi
...
if [ -z "$UPGRADE_KAFKA_STREAMS_TEST_VERSION" ]; then
  ...
  echo "VERS_NO_DOTS"
  echo "$VERSION_NO_DOTS"
  SHORT_VERSION_NO_DOTS=${VERSION_NO_DOTS%?} # remove last char, ie, bug-fix number
...
for file in "$base_dir"/shell/build/libs/kafka-shell*.jar; do
  if should_include_file "$file"; then
    CLASSPATH="$CLASSPATH":"$file"
  fi
done

for dir in "$base_dir"/shell/build/dependant-libs-${SCALA_VERSION}*; do
  CLASSPATH="$CLASSPATH:$dir/*"
done
...
for file in "$base_dir"/trogdor/build/libs/trogdor-*.jar; do
  if should_include_file "$file"; then
    CLASSPATH="$CLASSPATH":"$file"
  fi
done

for dir in "$base_dir"/trogdor/build/dependant-libs-${SCALA_VERSION}*; do
  CLASSPATH="$CLASSPATH:$dir/*"
done

for cc_pkg in "api" "transforms" "runtime" "mirror" "mirror-client" "json" "tools" "basic-auth-extension"; do
